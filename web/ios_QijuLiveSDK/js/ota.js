(function($) {
    ...
    var $BUILD = $("<li class='build'><a class='link'></a><span>[ <a class='bundle'>bundle资源</a> ]</span><span class='desc'></span><span class='date'></span><span><a class='dsym'></a></span></li>");
    
    OTA.$build = function(title, name, description, date) {
        if (!name) {
            return null;
        }
       ...
       $(".link", $build).attr("href", url + ".framework.zip");
        $(".bundle", $build).attr("href", url + ".bundle.zip");
        $(".desc", $build).html(description || "");
        $(".date", $build).html(date || "");
	$(".dsym", $build).text(" [.framework.dSYM] ");
        $(".dsym", $build).attr("href", url + ".framework.dSYM.zip");
    ...
    jQuery(function($) {
        ...
        for (var index in buildInfoList) {
            ...
            $(".content", $lastVersion).prepend(OTA.$build(buildInfo.appTitle, buildInfo.appFileName, buildInfo.appDescription,buildInfo.appDate));
        }
