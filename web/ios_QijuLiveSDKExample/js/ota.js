(function($) {
    ...
    var $BUILD = $("<li class='build'><a class='link'></a><span>[ <a class='apk'>IPA</a> ]</span><span class='desc'></span><span class='date'></span><span><a class='dsym'></a></span><span><a class='sdk'></a></span></li>");
    
    OTA.$build = function(title, name, description, date,sdkName) {
        if (!name) {
            return null;
        }
        ...
        url += name;
        var sdkUrl = OTA.URL;
        if (sdkUrl.charAt(sdkUrl.length -1) !== "/") {
            sdkUrl += "/";
        }
        sdkUrl += sdkName;
       ...
       $(".link", $build).attr("href", url + ".ipa");
        $(".apk", $build).attr("href", url + ".ipa");
        
        $(".desc", $build).html(description || "");
        $(".date", $build).html(date || "");
	$(".dsym", $build).text(" [.app.dSYM] ");
        $(".dsym", $build).attr("href", url + ".app.dSYM.zip");
        $(".sdk", $build).text("[ " + sdkName + " ]");
        $(".sdk", $build).attr("href", sdkUrl + ".framework.dSYM.zip");
        return $build;
    }
    ...
    jQuery(function($) {
        ...
        for (var index in buildInfoList) {
            ...
            $(".content", $lastVersion).prepend(OTA.$build(buildInfo.appTitle, buildInfo.appFileName, buildInfo.appDescription,buildInfo.appDate,buildInfo.sdkName));
        }
